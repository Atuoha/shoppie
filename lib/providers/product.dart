...
import 'package:shoppie/models/http_exception.dart';
...
class Products with ChangeNotifier {
  ...
  Future<void> deleteProduct(String id) async {
    var uri = Uri.parse(
        'https://fluttershoppie-default-rtdb.firebaseio.com/products/$id.json');

    try {
      http.delete(uri).then((response) => {
            if (response.statusCode >= 400)
              {
                throw HttpException('An error has occurred while deleting'),
              }
            else
  ...
  Future<void> fetchProduct() async {
    var uri = Uri.parse(
        'https://fluttershoppie-default-rtdb.firebaseio.com/products.json');
    ...
    try {
      ...
      final List<Product> loadProductData = [];
      extractedData.forEach((prodId, prodData) => {
            loadProductData.add(
              Product(
                ...
                previousPrice: prodData['previousPrice'],
                colors: prodData['colors'],
                isFavorite: prodData['isFavorite'],
              ),
            )
          });
      _availableProducts = loadProductData;
      notifyListeners();
    } catch (e) {
      // ignore: avoid_print
  ...
  Future<void> updateProduct(Product productDetails) async {
    final prodIndex = _availableProducts
        .indexWhere((product) => product.id == productDetails.id);
    if (prodIndex >= 0) {
      var url = Uri.parse(
          'https://fluttershoppie-default-rtdb.firebaseio.com/products/${productDetails.id}.json');
      ...
      try {
        ...
        _availableProducts[prodIndex] = productDetails;
      } catch (e) {
